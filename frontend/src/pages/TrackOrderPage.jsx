...
function TrackOrderPage() {
  ...
  // 🔹 DELIVERY BOY LIVE LOCATION LISTENER (cleanup added)
  useEffect(() => {
    if (!socket) return;

    const locationHandler = ({ deliveryBoyId, latitude, longitude }) => {
    ...
    socket.on("updateDeliveryLocation", locationHandler);

    return () => {
      socket.off("updateDeliveryLocation", locationHandler);
    };
  }, [socket]);

  // 🔹 🔥 FINAL FIX → ORDER STATUS REALTIME LISTENER
  useEffect(() => {
    if (!socket) return;
      ...
      setCurrentOrder((prev) => {
        ...
        return {
          ...
          shopOrders: prev.shopOrders.map((so) =>
            so.shop._id === data.shopId ? { ...so, status: data.status } : so,
          ),
        };
      });
    };
